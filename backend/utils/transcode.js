  ...
  return new Promise((resolve, reject) => {
    ffmpeg.getAvailableCodecs((err, codecs) => {
      ...
      if (missingCodecs.length > 0) {
        reject(
          new Error(`Missing required codecs: ${missingCodecs.join(", ")}`)
        );
        return;
      }
      resolve(codecs);
...
const transcodeVideo = async (inputPath, outputDir, videoId, onProgress) => {
  try {
    await verifyCodecs();
    ...
    for (const quality of qualities) {
      ...
      console.log(`Created quality directory: ${qualityDir}`);
    }
...
360p/${videoId}.m3u8`;
    ...
    await fsPromises.writeFile(masterPlaylistPath, masterPlaylist);
          ...
          .on("progress", (progress) => {
            if (onProgress) {
              ...
              const qualityProgress =
                (progress.percent || 0) / qualities.length;
    ...
    console.log(`All qualities transcoded for video ${videoId}`);
    ...
    console.log(`All output files verified for video ${videoId}`);
    return path.relative(process.cwd(), masterPlaylistPath);
  } catch (error) {
    console.error(`Transcoding error for video ${videoId}:`, error);
    throw new ApiError(500, "Error transcoding video", error);
  }
};
