...
exports.uploadVideo = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    // Create a permanent location for the original video file
    const videoDir = path.join("uploads", "videos", videoId.toString());
    await fsPromises.mkdir(videoDir, { recursive: true });

    // Move the temp file to the permanent location
    const originalFilePath = path.join(
      videoDir,
      `${videoId}${path.extname(videoFile.originalname)}`
    );
    await fsPromises.rename(tempPath, originalFilePath);
    ...
    const video = await Video.create({
      ...
      filePath: relativeFilePath, // Store the original file path
    ...
    await transcodeQueue.add(
      "transcode",
      {
        inputPath: originalFilePath, // Use the permanent file path
        outputDir,
...
exports.streamVideo = asyncHandler(async (req, res, next) => {
  ...
  // If HLS path doesn't exist, try to use the original video file
  if (!hlsPath) {
    console.log(`No hlsPath for video: ${videoId}, checking for original file`);

    // Check if original file exists
    if (video.filePath) {
      const originalFilePath = path.join(process.cwd(), video.filePath);
      console.log("Attempting to stream original file:", originalFilePath);
    ...
    // If we get here, neither HLS nor original file is available
    console.error(`No hlsPath or original file for video: ${videoId}`);
    return next(
      new ApiError(
        "Video stream not ready. Transcoding may be incomplete or the original file is missing.",
        422
      )
    );
  }
...
exports.deleteVideo = asyncHandler(async (req, res, next) => {
  ...
  if (video.uploader.toString() !== userId && req.user.role !== "admin") {
    return next(new ApiError("Unauthorized to delete this video", 403));
  }
  ...
  try {
    ...
    if (video.hlsPath) {
      const hlsDir = path.dirname(path.join(process.cwd(), video.hlsPath));
    ...
    return next(new ApiError(`Failed to delete video: ${error.message}`, 500));
  }
});
